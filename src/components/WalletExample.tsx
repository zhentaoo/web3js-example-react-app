...
export const WalletExample = () => {
  ...
  const getToAddressBalance = useCallback(
    (address?: string) => {
        ...
        <div>
          <div className="Block">
            <Input
              ...
              value={pk}
              onChange={createNewWallet}
            />
            {wallet && (
              <>
                <div>You address: {wallet && wallet.getAccount().address}</div>
                {balance && <div>Your balance: {balance}ETH</div>}
              </>
            )}
          </div>
          <div className="Block">
            {balance && (
              <div>
                <div className="Block">
                  <Input
                    label="Send 0.001ETH to this address:"
                    value={toAddress}
                    onChange={handleChangeToAddress}
                  />
                </div>
                <div className="Block">
                  <button type="button" onClick={handleSendEther}>
                    SEND ETHER
                  </button>
                </div>
                {toAddressBalance && (
